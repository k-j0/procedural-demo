...
float4 main(FS_IN input) : SV_TARGET{
	...
	slope = slope * 4 - 1;
	float3 baseColour = float3(0.3f,0.3f,0.3f);
	float3 stone = texDiffuse.Sample(sampler0, input.tex).rgb;
	baseColour = stone;
	//increase brightness based on height
	float heightFactor = saturate((input.worldPosition.y + 10)*0.05f);
	baseColour *= heightFactor;
	//baseColour += float3(0.25f, 0.22f, 0.2f);
	...
	float2 causticsUvs = fmod((input.tex+input.worldPosition.xy) * 0.06f + float2(sin(time*0.004f) * 10, cos(time*0.009f) * 10), 1);//scale the uvs up to get smaller caustics
